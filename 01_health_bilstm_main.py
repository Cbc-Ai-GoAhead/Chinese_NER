...
from package.model import BiLSTM_CRF, BiLSTM_CRF_01_dropout
from package.dataset import HealthDataset, decode_tags_from_ids
...
parser.add_argument("--Train_data_path",default="data/health_train_character.txt",type=str,help="設定 Training data path")
#設定 Evaluation data path
parser.add_argument("--Eval_data_path",default="data/health_eval_character.txt",type=str,help="設定 Evaluation data path")
#設定 Testing data path
parser.add_argument("--Test_data_path",default="data/health_test_character.txt",type=str,help="設定 Testing data path")
#設定 Training Epoch
...
#pretrai nembedding
parser.add_argument("--embedding",default='embedding/health_word2vec.txt',type=str,help="which gpu")
#embedding dimesion
...
if mode == 'Train':
    ...
    dataset_train = HealthDataset(Train_data_path,word2vec,dimesion,mode='Train')
    ...
    dataset_eval = HealthDataset(Eval_data_path,word2vec,dimesion,mode='Eval')
    ...
    #Create Model
    model = BiLSTM_CRF_01_dropout(len(dataset_train.id2tag),lstm_hidden_dim=lstm_hidden_dim,lstm_dropout_rate=lstm_dropout_rate).to(device)
    ...
    for epoch in range(EPOCH):
        ...
        with tqdm(desc='Train', total=len(dataloader_train)) as t:
            for i, (input,mask,label) in enumerate(dataloader_train):
                ...
                loss = model(input,mask,label)#crf.neg_log_likelihood_loss
                # loss is a numpy array
...
#Predict Mode
    ...
    dataset_test = HealthDataset(Test_data_path,word2vec,dimesion,mode='Test')
